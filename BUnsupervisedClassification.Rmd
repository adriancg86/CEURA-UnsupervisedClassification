...
* Un punto $p$ es un punto núcleo si al menos $minPts$ puntos están a
una distancia $\epsilon$ de él. 
* Un punto $q$ se dice directamente alcanzable desde el punto núcleo $p$ si 
está a una distancia menor que $\epsilon$ de $p$. Si existen más de 
$minPts$ en un entorno $\epsilon$ de $p$, decimos que esos puntos son 
directamente alcanzables desde $p$. Por definición, no es posible tener puntos 
directamente alcanzables desde un punto que no sea un núcleo.
* Un punto $q$ es alcanzable desde $p$ si existe una secuencia de
puntos $p_1,p_2,...,p_n$ donde $p=p_1$ y $p_n = q$ tal que cada 
punto $p_{i+1}$ es directamente alcanzable desde $p_i$; es 
decir, todos los puntos de la secuencia deben ser puntos núcleos, con 
la posible excepción de q {\displaystyle q} q.
* Un punto que no sea alcanzable desde cualquier otro punto es 
considerado ruido.
* Si $p$ es un punto núcleo, éste forma un cluster junto a otros puntos
(núcleo o no) que sean alcanzables desde él. Cada cluster contiene al 
menos un punto núcleo. Los puntos que no sean puntos núcleo alcanzables 
pueden pertenecer a un cluster pero actúan como una barrera puesto que 
no es posible alcanzar más puntos desde estos.
* La relación ser alcanzable no es simétrica. 
* Última definición necesaria: conectividad. Dos puntos $p$
 y $q$ están conectados densamente si existe un punto $o$ tal que ambos $p$
 y $q$ sean directamente alcanzables desde $o$. La relación estar densamente
 conectado es simétrica.
*Un clúster en DBSCAN, satisface dos propiedades:
** Todos los puntos del clúster están densamente conectados entre sí.
** Si un punto A es directamente alcanzable desde cualquier otro punto B del clúster, entonces A también forma parte del clúster.
